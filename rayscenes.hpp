...
public:
  /**
   * @brief Type for the function for a rayscene, expands to `void
   * (*)(rayscenes::rayscenemanager&, bool)`
   *
   */
  typedef void (*rayscenefunc)(rayscenes::rayscenemanager &, bool);

  /**
   * @brief RayScene class, manages a single scene, it is mostly an abstraction
   * for the scene functions
   *
   */
  class rayscene {
  public:
     ...
     *
     * @param _func The function to call when the scene is active and should be
     * rendered
     */
  ...
  private:
    ...
    /**
     * @brief The function that will be called when the scene is active and
     * should be rendered
     *
     */
    rayscenefunc func;
   ...
   *
   * @return std::unordered_map<std::string, rayscene>& Reference to map
  ...
  /**
   * @brief Clears all scenes from the list, effectively stopping rendering. It
   * is recommended you always have one scene active or some other update loop
   * to ensure the program stays working.
   *
   * @param id The target id
   */
  ...
  /**
   * @brief Renders all scenes that are active, it is safe to modify the list
   * during this running as it first creates a copy on the deque before
   * beginning rendering.
   *
   */
...
private:
   ...
   *
   * @tparam T The type the deque stores, in general it is able to be deduced by
   * the compiler
   * @param q The deque to clear
   */
  template <typename T> void clear_deque(std::deque<T> &q);

  /**
   * @brief Clears a map of entries
   *
   * @tparam K The type used for the key, in general it is able to be deduced by
   * the compiler
   * @tparam V The type used for the value, in general it is able to be deduced
   * by the compiler
...
// Initializers
//
rayscenemanager::rayscene::rayscene(rayscenefunc _func) {
  func = _func;
  funcSet = true;
}
...
// Utilities
//
void rayscenemanager::rayscene::setFunc(rayscenefunc _func) {
  func = _func;
  funcSet = true;
}
...
rayscenemanager::rayscenemanager(const rayscenemanager &r) {
  ...
  for (auto &[key, scene] : ((rayscenemanager)r).getAllScenes())
    scenes[key] = scene;
  for (auto &scene : ((rayscenemanager)r).getActiveScenes())
...
//
std::unordered_map<std::string, rayscenemanager::rayscene> &
rayscenemanager::getAllScenes() {
  return scenes;
}
void rayscenemanager::setAllScenes(
    std::unordered_map<std::string, rayscene> newScenes) {
  for (auto &[key, scene] : newScenes)
    scenes[key] = scene;
}
...
//
std::deque<std::string> &rayscenemanager::getActiveScenes() {
  return activeScenes;
}
...
void rayscenemanager::removeActiveScene(std::string id) {
  activeScenes.erase(std::remove(activeScenes.begin(), activeScenes.end(), id),
                     activeScenes.end());
}
void rayscenemanager::removeActiveScenes(std::deque<std::string> ids) {
  for (auto &id : ids)
    activeScenes.erase(
        std::remove(activeScenes.begin(), activeScenes.end(), id),
        activeScenes.end());
}
...
//
template <typename T> void rayscenemanager::clear_deque(std::deque<T> &q) {
  std::deque<T> empty;
  std::swap(q, empty);
}
template <typename K, typename V>
